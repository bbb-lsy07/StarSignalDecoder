...
> **重要提示**：请**不要**直接通过 `curl ... | sh` 或 `wget ... | sh` 运行脚本。这可能导致输入阻塞（如菜单选项无法响应）。请务必**下载脚本到本地后运行**，以确保交互正常。
...
2.  **运行脚本**：
    ...
    # 或者在 PowerShell 中直接运行，脚本会尝试使用 sh
    # sh starsignal_manager.sh
    ```
...
安装 Python：
推荐使用 winget (Windows 10 1709+ 内置) 或从 Python 官网 下载安装，务必勾选 "Add Python to PATH"。
...
安装 Git for Windows：
推荐使用 winget 或从 Git for Windows 官网 下载安装，在安装时勾选 "Use Git from the Windows Command Prompt" 或 "Git from the command line and also from 3rd-party software"。
...
手动添加 PATH (如果未自动添加)：
如果 starsignal 命令无法找到，可能需要手动添加 Python Scripts 路径到系统 PATH 环境变量。
找到类似 C:\Users\你的用户名\AppData\Roaming\Python\Python39\Scripts 的路径，并将其添加到用户环境变量的 Path 中。
在 PowerShell 中可以运行：

# 假设你安装的是 Python 3.9，路径可能因版本而异
...
easy：简单模式，60秒，3选项，能量损失少。

medium：中等模式，45秒，4选项，能量损失中等。

hard：困难模式，30秒，5选项，能量损失高。

challenge：挑战模式，随机规则，40秒，4选项，能量损失中等。
...
提交 Pull Request 到 main 分支，使用 PR 模板。
...
避免管道运行 (curl ... | sh)：请务必下载脚本到本地后运行，如 快速开始 所示。
...
同时确保已安装 colorama: pip install --user colorama。

starsignal 命令未找到

问题：运行 starsignal 提示命令不存在。

解决：通常是 Python 的 Scripts 目录未添加到 PATH 环境变量。管理脚本会尝试修复。手动修复：
...
解决：检查网络连接，并尝试使用公共 DNS（如 Google DNS 8.8.8.8）：
...
Q：存档丢失怎么办？
A：检查存档路径和权限（见 存档无法加载或保存），如果问题依旧，请联系作者。
