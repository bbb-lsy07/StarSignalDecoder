...
- **沉浸剧情**：包含 NPC 任务、随机事件（风暴、商人），支持多结局（星际传奇、险象环生、信号失联）。
- **美观界面**：
...
```bash
# 直接运行
curl -s https://raw.githubusercontent.com/bbb-lsy07/StarSignalDecoder/main/starsignal_manager.sh | sh
# 或
wget -qO- https://raw.githubusercontent.com/bbb-lsy07/StarSignalDecoder/main/starsignal_manager.sh | sh
```
...
### 要求
- **Python 3.6+**：运行 `python3 --version`（Windows：`python --version`）检查。
- **pip**：Python 包管理工具。
- **git**：用于从 GitHub 下载代码。
- **可选**：安装 `colorama` 以启用彩色输出：
  ```bash
