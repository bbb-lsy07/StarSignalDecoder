...
detect_language() {
    ...
    if [ -n "$LANG" ] && echo "$LANG" | grep -qi "zh"; then
        LANG_MODE="zh"
    elif [ "$(uname)" = "Darwin" ] && defaults read NSGlobalDomain AppleLanguages | grep -q "zh"; then
        LANG_MODE="zh"
    elif [ -n "$SYSTEMROOT" ] && powershell -Command "Get-Culture" | grep -qi "zh"; then
        LANG_MODE="zh"
    else
        LANG_MODE="en"
    fi
...
translate() {
    ...
    if [ "$LANG_MODE" = "zh" ]; then
        case "$key" in
            ...
            "installing") echo "正在安装 %s 分支...";;
            "updating") echo "正在更新到 %s 分支...";;
            ...
            "python_not_found") echo "未找到 Python3 或版本过旧（需要 >= %s）。是否安装？（y/n）：";;
            ...
            "progress") echo "[%s] 正在处理...";;
            "error") echo "错误：%s";;
            "check_log") echo "请查看 %s 获取详细信息";;
            "confirm_action") echo "是否继续执行 %s？（y/n）：";;
            *) echo "$key";;
        esac
    else
        case "$key" in
            ...
            "installing") echo "Installing %s branch...";;
            "updating") echo "Updating to %s branch...";;
            ...
            "python_not_found") echo "Python3 not found or outdated (need >= %s). Install? (y/n):";;
            ...
            "progress") echo "[%s] Processing...";;
            "error") echo "Error: %s";;
            "check_log") echo "Check %s for details";;
            "confirm_action") echo "Confirm to proceed with %s? (y/n):";;
            *) echo "$key";;
        esac
    fi
}
...
detect_os() {
    if [ -n "$SYSTEMROOT" ]; then
        OS="windows"
        VERSION=$(powershell -Command "[System.Environment]::OSVersion.Version" | awk '{print $1}')
    ...
    case "$OS" in
        ...
        windows)
            if command_exists winget; then
                winget install --id Python.Python.3 --version 3.9.13 || die "$(translate "error" "Python3 安装失败")"
            elif command_exists choco; then
                choco install python --version 3.9.13 || die "$(translate "error" "Python3 安装失败")"
            else
                log "正在安装 winget..."
                powershell -Command "Invoke-WebRequest -Uri https://github.com/microsoft/winget-cli/releases/latest/download/Microsoft.DesktopAppInstaller_8wekyb3d8bbwe.msixbundle -OutFile winget.msixbundle; Add-AppxPackage winget.msixbundle" || die "$(translate "error" "winget 安装失败")"
                winget install --id Python.Python.3 --version 3.9.13 || die "$(translate "error" "Python3 安装失败")"
            fi
            ;;
        *)
...
install_pip() {
    ...
    if check_python; then
        ...
        if [ "$OS" = "windows" ]; then
            export PATH="$HOME/AppData/Roaming/Python/Python39/Scripts:$PATH"
        else
...
install_git() {
    ...
    case "$OS" in
        ...
        windows)
            if command_exists winget; then
                winget install --id Git.Git || die "$(translate "error" "git 安装失败")"
            elif command_exists choco; then
...
fix_path() {
    ...
    if [ "$OS" = "windows" ]; then
        ...
        powershell -Command "[Environment]::SetEnvironmentVariable('Path', [Environment]::GetEnvironmentVariable('Path', 'User') + ';$PYTHON_SCRIPTS', 'User')" || die "$(translate "error" "无法更新 PATH")"
...
check_starsignal() {
    if command_exists starsignal; then
        ...
        if [ -n "$PIP_INFO" ]; then
            INSTALLED_BRANCH=$(echo "$PIP_INFO" | grep -i "Location" | grep -o "git+.*@.*" | grep -o "@.*" | cut -d@ -f2)
...
main_menu() {
    ...
    if check_starsignal; then
        ...
        case "$choice" in
            ...
            5) fix_environment ;;
            6) log "退出程序"; exit 0 ;;
            *) log "无效选项"; echo "$(translate "invalid_choice")"; sleep 1; main_menu ;;
        esac
    else
...
while [ $# -gt 0 ]; do
    case "$1" in
        --lang) detect_language "$1" "$2"; shift 2 ;;
        *) shift ;;
    esac
done
